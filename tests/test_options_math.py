...
Each test checks a property whose failure would make the scanner's numbers
quietly wrong rather than crash it - the most dangerous class of bug in an
options tool.
"""
...
# A typical BTC option: spot 100k, 30 days, 60% vol.
S = 100_000.0
T30 = 30.0 / 365.0
SIG = 0.60
...
def test_put_call_parity_holds_at_zero_rate():
    """C - P = S - K when r = 0. Any sign flip in the pricing is caught here."""
...
def test_degenerate_inputs_return_nan_not_crash(S_, K_):
    """A live chain contains junk rows; the scanner must not crash on them."""
...
def test_delta_bounds():
    for K in (50_000.0, 100_000.0, 200_000.0):
        ...
        assert 0.0 <= c <= 1.0
        assert -1.0 <= p <= 0.0
        # Delta parity: call delta - put delta = 1 (at r = 0)
        assert c - p == pytest.approx(1.0, abs=1e-9)


def test_delta_matches_finite_difference():
    """Analytic delta == numerical dPrice/dSpot. A formula typo is caught here."""
    K, h = 105_000.0, 1.0
    for is_call in (True, False):
...
def test_decay_schedule_spans_full_life_without_exploding():
    """A long expiry should widen the step rather than truncate the schedule."""
    ...
    # cum_decay must increase monotonically
...
def test_implied_vol_returns_nan_below_intrinsic():
    """A stale quote implying arbitrage must return NaN, not a junk number."""
...
def _samples(divisor: float) -> list:
    """A synthetic chain whose IV is supplied at a known scale."""
    out = []
...
def test_breakevens_on_simple_v_shape():
    ...
    payoff = [abs(s - 100.0) - 5.0 for s in spots]   # crosses at 95 and 105
...
def test_breakeven_exactly_on_grid_point_is_not_double_counted():
    """Straddle break-evens land on round numbers, so de-duplication matters."""
...
def test_flat_zero_segment_yields_one_breakeven_not_many():
    """A capped structure sitting at exactly zero across a whole range."""
    spots = [90.0, 95.0, 100.0, 105.0, 110.0]
...
def test_years_to_expiry_uses_exact_seconds_not_whole_days():
    """On expiry day, whole-day rounding overstates premium roughly 8x."""
...
def test_short_straddle_reports_unlimited_loss_not_unlimited_profit():
    """The most dangerous error: showing a short straddle's max loss as finite."""
...
def test_long_straddle_reports_unlimited_profit_not_unlimited_loss():
    """The mirror case: a long straddle's loss is capped at the premium paid."""
    p = _payoff(GRID, [(100_000.0, True, 1), (100_000.0, False, 1)], -8_000.0)
    tail = om.payoff_tail_risk(p)
...
def test_short_call_unlimited_loss_lives_on_the_UPSIDE():
    """A check that looks only at the left edge misses this entirely."""
...
def test_iron_condor_is_bounded_on_both_sides():
    """On a defined-risk structure any 'Unlimited' badge is wrong."""
...
def test_tail_risk_tolerance_scales_with_payoff_size():
    """Floating-point noise on a large flat payoff must not read as unlimited."""
    flat = [250_000.0] * 401
